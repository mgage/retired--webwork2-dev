...
# $CVSHeader: webwork2/lib/WeBWorK/ContentGenerator/GatewayQuiz.pm,v 1.48.2.1 2007/08/13 22:53:45 sh002i Exp $
# 
...
use RECORD;  # <---  Temp log file printer to enable debugging thru text file output
...
sub pre_header_initialize {
	...
	my $EffectiveUser = $db->getUser($effectiveUserName); die "record for user $effectiveUserName (effective user) does " .	"not exist." unless defined $EffectiveUser;
	my $PermissionLevel = $db->getPermissionLevel($userName); die "permission level record for $userName does not exist (but the " ."user does? odd...)" unless defined($PermissionLevel);
	...
	####################################################################################################
	#### 
	####	PRINT SOME VARIABLES TO PLAY WITH
	####
